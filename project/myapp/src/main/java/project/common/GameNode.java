...
/**
 * The GameNode class represents a node in the game grid, which could be a part
 * of a wire, a bulb, or a power source.
 * It keeps track of its position, the sides it connects to, and whether it is
 * powered on or off.
 * This class extends AbstractObservableField, meaning it can notify observers
...
public class GameNode extends AbstractObservableField {
    ...
    /**
     * Constructor for creating a GameNode with a specific position, type, and
     * sides.
     * 
    ...
    /**
     * Constructor for creating a GameNode with a specific position and type (no
     * sides).
     * 
     * @param row  The row of the node.
     * @param col  The column of the node.
     ...
     * This changes the direction of all the connectors and updates the number of
     * turns.
     */
    public void turn() {
        int i = 0;
    ...
    public void turnReverse() {
        int i = 0;
        for (Side side : this.side) {
            int currentIndex = side.ordinal();
            this.side[i] = Side.values()[(currentIndex + 3) % Side.values().length];
            i++;
        }
        numberOfTurns--;
        List<Side> order = Arrays.asList(Side.NORTH, Side.EAST, Side.SOUTH, Side.WEST);
    ...
    public void setPowered(boolean powered) {
        ...
        // notifyObservers(); // Uncomment if observers need to be notified when the
        // power state changes
    }
     ...
     * The method checks which sides (NORTH, EAST, SOUTH, WEST) are connected and
     * returns
     ...
     * @throws IllegalStateException if no matching shape is found (unexpected
     *                               value).
     */
    ...
    /**
     * Returns a string representation of the GameNode, including its type,
     * position, and connected sides.
     * 
     * @return A string representation of the GameNode.
     */
    public String toString() {
        return String.format("%s %d %d %s",
                type.swichSideToString(),
                position.getRow(),
                position.getCol(),
                String.join(" ", Arrays.stream(side).map(Enum::name).toArray(String[]::new)));
    }
